...
impl <T: Borrow<RndTable>> Random<T> {

	pub fn next(&mut self) -> i32 {
		self.index = self.index.wrapping_add(1);
		self.table.borrow()[self.index as usize] as i32
	}

	pub fn next_sub(&mut self) -> i32 {
		self.next() as i32 - self.next() as i32
	}
...
#[unsafe(no_mangle)]
pub extern "C" fn CreateRandom() -> Random<&'static RndTable> {
	Random::new(&RND_TABLE)
}
...
mod test {
	...
	#[fixture]
	fn random_table() -> RndTable {
		[0u8; 256]
	}

	#[rstest]
	fn get_next_random_value(mut random_table: RndTable) {
	...
	#[rstest]
	fn get_next_sub_random_value(mut random_table: RndTable) {
	...
	#[rstest]
	fn clear_random(mut random_table: RndTable) {
	...
	#[rstest]
	fn get_next_random_value_wraps_around(mut random_table: RndTable) {
